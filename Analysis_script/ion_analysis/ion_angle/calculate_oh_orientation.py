...
"""
Calculate the radial orientation distribution of hydroxide (OH-) ions relative to the bubble interface.
...
def determine_files_to_process(start_ns, end_ns, time_step_ns):
    ...
    e.g., start=14, end=30, time_step=8 -> ['8-16ns', '16-24ns', '24-32ns']
    """
    
    # Determine first file index containing start time
    start_file_index = int(np.floor(start_ns / time_step_ns))
    # Determine last file index containing end time
    # Uses ceil(end_ns / time_step_ns) - 1
    end_file_index = int(np.ceil(end_ns / time_step_ns)) - 1
    
    # If end_ns is exactly an integer multiple of time_step_ns (e.g. 16.0), np.ceil gives 2, minus 1 is 1 (8-16ns).
    # But data for 16.0ns is at the beginning of the 16-24ns file.
    # To handle this boundary case, we check if end_ns is an integer multiple
    if end_ns % time_step_ns == 0:
        pass 

    if start_file_index > end_file_index:
         end_file_index = start_file_index # Ensure at least one file is processed
...
def load_interface_map(time_ranges, log_path_template, bubble_name, block_step_ns):
    ...
    """
    interface_map = {}
    blocks_per_file = int(args.time_step_ns / block_step_ns) # 8ns / 1ns = 8
    
    print("--- Loading Interface Position Logs ---")
    
    for time_range in time_ranges:
        file_start_ns = float(time_range.split('-')[0]) # e.g., 8.0
        
        for i in range(blocks_per_file):
            block_index = i + 1
            current_block_start_ns = file_start_ns + (i * block_step_ns) # e.g., 8.0, 9.0, ...
            ...
            if interface_pos is not None:
                # Key is the start time of the block
...
def process_ion_file(ion_file_path, interface_map, all_data_by_bin, dist_bin_edges,
    ...
    for row_index, line in enumerate(lines):
        ...
        # Calculate global time for current row
        current_time_ns = file_start_ns + (row_index * row_time_step)
        
        # Check if within user specified total time range
        if not (start_ns <= current_time_ns < end_ns):
            continue
            
        # Find interface position for this time
        # e.g., 14.234 ns -> find key 14.0
        ...
        if interface_pos is None:
            # Log file might be missing, skip this frame
            continue
            
        parts = line.split()
        if len(parts) < 10: # At least N_ions, MD_step and 1 ion (8 columns)
            continue
        ...
        try:
            ...
            for i in range(n_ions):
                # Each OH- ion has 8 columns: O_ID, Ox, Oy, Oz, H_ID, Hx, Hy, Hz
                offset = 2 + i * 8
                if offset + 7 >= len(parts):
                    break # Incomplete row data

                o_pos = np.array([float(parts[offset+1]), float(parts[offset+2]), float(parts[offset+3])])
                h_pos = np.array([float(parts[offset+5]), float(parts[offset+6]), float(parts[offset+7])])
                
                # 1. Calculate relative distance (r)
                dist_to_center = np.linalg.norm(o_pos)
                # Positive inside bubble (r > 0), negative outside bubble (r < 0)
                relative_dist = interface_pos - dist_to_center
                
                # 2. Calculate orientation angle (angle)
                # Vector 1: H -> O
                vec_ho = o_pos - h_pos
                # Vector 2: O -> Center (0,0,0)
                vec_o_center = -o_pos 
                
                angle = calculate_angle_between_vectors(vec_ho, vec_o_center)
...
def main():
    parser = argparse.ArgumentParser(description="Calculate OH- Ion Radial Orientation Distribution")
    ...
    parser.add_argument("--ion_path_template", type=str, required=True, 
                        help="Ion file path template, using {time_range} and {bubble_name} placeholders. "
                             "Example: '/.../findion/{bubble_name}/oh_..._{bubble_name}_{time_range}.txt'")
    ...
    parser.add_argument("--output_prefix", type=str, default="oh_orientation_results", 
    ...
    # Use defaultdict so we don't have to check if bin_key exists
    # all_data_by_bin = {bin_center: [list_of_angles]}
    all_data_by_bin = defaultdict(list)
    for bin_center in dist_bin_centers:
        all_data_by_bin[bin_center] # Ensure all bins are initialized
    ...
    print(f"\nTotal processing complete. Collected {total_ions} OH- ion data points.")
    if total_ions == 0:
    ...
    header = (f"# OH- Mean Orientation vs. Distance ({args.start_ns}-{args.end_ns} ns)\n"
              f"# Bubble: {args.bubble_name}\n"
              f"# Angle: (H->O vector) vs (O->Center vector)\n"
    ...
    # Only plot where there are data points
    ...
    plt.ylabel("Mean OH⁻ Orientation Angle (deg)")
    plt.title(f"OH⁻ Mean Orientation vs. Distance ({args.bubble_name})")
    plt.legend()
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    
    # Set reasonable Y axis range (e.g., 0-180)
    plt.ylim(0, 180)
