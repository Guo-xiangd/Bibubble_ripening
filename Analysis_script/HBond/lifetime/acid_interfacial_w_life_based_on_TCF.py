...
# -*- coding: utf-8 -*-
"""
Acid System: Residence-Coupled Hydrogen Bond (HB) Dynamics.
Includes dynamic H3O+ ion shielding mechanism to isolate pure water HBs.
"""
...
CONFIG = {
    ...
    "traj_start_time_ns": 32.0,  
    "dt_ps": 0.1,                
    
    # H3O+ ion tracking file
    "h3o_file": "./data/h3o_ions.txt",
    ...
    # Output settings
    "out_txt": "acid_water_HB_Res_Lifetimes_2.txt",
    "out_img": "acid_water_HB_Res_Lifetimes_2.png"
}
...
def load_h3o_info(filepath):
    """
    Parses the H3O+ file, returning sets of excluded O and H IDs per frame.
    Format: { frame_idx: set(IDs) }
    """
    print(f"Parsing H3O+ topology file: {filepath} ...")
    h3o_o_ids = {}
    h3o_h_ids = {}
    
    if not os.path.exists(filepath):
        print(f"[Error] H3O+ file not found: {filepath}")
        return h3o_o_ids, h3o_h_ids
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            for i in range(n_ions):
                ...
                h1_id = int(parts[base_idx + 4])
                h2_id = int(parts[base_idx + 8])
                h3_id = int(parts[base_idx + 12])
                
                o_set.add(o_id)
                h_set.update([h1_id, h2_id, h3_id])
                
            h3o_o_ids[md_step] = o_set
            h3o_h_ids[md_step] = h_set
            
    print(f"[Success] Loaded {len(h3o_o_ids)} frames of H3O+ data.")
    return h3o_o_ids, h3o_h_ids
...
def find_all_hbs_and_residence(o_atoms, h_atoms, z_int, z_range, exclude_o_ids, exclude_h_ids):
    """Find current frame pure water HBs at the interface (excluding ions)."""
    ...
    for p in pairs_wh:
        h_global_idx = p[1]
        h_id = h_ids[h_global_idx]
        
        if h_id in exclude_h_ids:
            continue 
...
def main():
    h3o_o_dict, h3o_h_dict = load_h3o_info(CONFIG['h3o_file'])
    ...
    print("Starting TCF forward scan (H3O+ excluded)...")
    ...
    for ts in u.trajectory:
        ...
        exclude_o = h3o_o_dict.get(curr_frame, set())
        exclude_h = h3o_h_dict.get(curr_frame, set())
    ...
    header = (
        "Acid System (Water-Water HB) Residence-Coupled Dynamics\n"
    ...
    ax.plot(t_axis, C_intermittent, '-', color='#d62728', lw=2.5, label='Intermittent')
    ax.plot(t_axis, C_continuous, '--', color='#1f77b4', lw=2.5, label='Continuous')
    ...
    ax.set_title("Acid System: Water-Water HB Dynamics")
    ax.grid(alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(CONFIG['out_img'], dpi=300)
    print(f"[✔] Run complete! Results saved.")
